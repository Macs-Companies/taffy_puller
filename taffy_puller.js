...
async function processOrder(orderNumber,schedule){
  ...
  if(processType === 'SOCK'){
    destination = `~/Desktop/_SOCKS/${orderInfo.so} ${orderInfo.customer.replace(/\//g,' ')}/`
  }else {
    destination = `~/Desktop/_PRINTPREP/_${processType}/${orderInfo.so} ${orderInfo.customer.replace(/\//g,' ')}/`
  }
